...
import { Check, ChevronLeft, AlertTriangle, ArrowRight, Pencil, X } from 'lucide-react';
...
export default function TransactionReview({ data, accounts, accountId, onBack, onDone }) {
  const { transactions: raw, bankName, accountName, period, source } = data;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0, minWidth: 0 }}>
      ...
      {/* Source badge */}
      {source === 'image' && (
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 12, padding: '7px 12px', borderRadius: 'var(--radius-sm)', background: 'var(--color-bg-elevated)' }}>
          <AlertTriangle size={12} style={{ color: 'var(--color-chart-warm)', flexShrink: 0 }} />
          <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
            Parsed via AI — please review dates and amounts carefully before importing.
          </p>
        </div>
      )}
